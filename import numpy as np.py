...
print("=== FIR THÔNG THẤP - WINDOW METHOD ===")
delta = float(input("Nhập độ gợn δ (vd 0.002): "))
wp_input = float(input("Nhập wp (theo π, vd 0.4): "))
ws_input = float(input("Nhập ws (theo π, vd 0.6): "))
wp = wp_input * np.pi
ws = ws_input * np.pi
#suy hao
A = -20 * np.log10(delta)
